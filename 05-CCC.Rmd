...
```{r load_data}
brain_ant <- readRDS("datasets/cluster_mouse_brain.rds")
...
scalefactors = jsonlite::fromJSON(txt = file.path("datasets/mouse_brain_spatial/",
...
saveRDS(cellChat, file = "datasets/cellchat_visium_mouse_cortex.rds")
```
...
```{r vis_full}
cellChat <- readRDS("datasets/cellchat_visium_mouse_cortex.rds")

# These aren't ggplot objects so we have to save them a bit differently
png(filename = paste0("Figure_ccc_", fig_num, ".png"), width = two_panel*2, height = two_panel*2, units = "in", res = 300)
...
png(filename = paste0("Figure_ccc_", fig_num, "a.png"), width = two_panel*1.5, height = two_panel, units = "in", res = 300)
par(mfrow = c(1,1), xpd=TRUE)
...
png(filename = paste0("Figure_ccc_", fig_num, "b.png"), width = two_panel*1.5, height = two_panel, units = "in", res = 300)
par(mfrow = c(1,1), xpd=TRUE)
...
# Circle plot
png(filename = paste0("Figure_ccc_", fig_num, ".png"), width = two_panel*1.5, height = two_panel*1.5, units = "in", res = 300)
...
# spatial plot - This comment will not run at the moment. I am working on trying to get an answer on the CellChat github.
#par(mfrow=c(1,1), xpd = TRUE)
...
png(filename = paste0("Figure_ccc_", fig_num, ".png"), width = two_panel*2, height = two_panel, units = "in", res = 300)
par(mfrow=c(1,1))
...
# This next plot is also broken. Maybe CellChat will fix it at some point...
